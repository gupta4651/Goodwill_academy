...
export default function Contact() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const enquiry = [
      'Hi Goodwill Classes, I want admission details.',
      `Name: ${formData.name}`,
    ...
    <section id="contact" className="bg-white py-16 lg:py-24">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="mx-auto mb-12 max-w-3xl text-center">
          ...
          <h2 className="section-title">Book a free demo class.</h2>
          <p className="mt-5 text-lg leading-8 text-slate-600">
            Share the student details and the academy team can follow up with batch timings, fees, and demo class availability.
          </p>
        </div>

        <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-lg border border-slate-200 bg-[#f8f4ea] p-5 shadow-sm sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5">
              <div>
                <label htmlFor="name" className="mb-2 block text-sm font-black text-slate-800">Full Name *</label>
                <input
                  id="name"
                  type="text"
                  required
                  ...
                  className="w-full rounded-md border border-slate-300 bg-white px-4 py-3 outline-none transition focus:border-red-700 focus:ring-2 focus:ring-red-100"
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <div>
                  <label htmlFor="phone" className="mb-2 block text-sm font-black text-slate-800">Phone *</label>
                  <input
                    id="phone"
                    type="tel"
                    required
              ...
              <div>
                <label htmlFor="course" className="mb-2 block text-sm font-black text-slate-800">Interested Course *</label>
                <select
                  id="course"
                  required
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-md border border-slate-300 bg-white px-4 py-3 outline-none transition focus:border-red-700 focus:ring-2 focus:ring-red-100"
                  placeholder="Preferred batch timing, class, exam target, or questions"
                />
              </div>

              <button type="submit" disabled={isSubmitting} className="btn-primary w-full disabled:cursor-not-allowed disabled:opacity-60">
                {isSubmitting ? (
                  <><span className="h-5 w-5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Preparing...</>
                ) : (
                  <><Send className="h-5 w-5" /> Send on WhatsApp</>
                )}
              </button>
            </form>
          </div>
          ...
          <div className="grid gap-5">
            <a href={`tel:${phoneHref}`} className="rounded-lg bg-slate-950 p-6 text-white shadow-xl shadow-slate-900/15 transition hover:-translate-y-1">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-red-700">
            ...
            <a
              ...
              className="flex items-center justify-center gap-3 rounded-lg bg-emerald-600 px-6 py-4 font-black text-white transition hover:-translate-y-1 hover:bg-emerald-700"
            >
